...
class TestProcessLogFile:
    ...
    async def test_processes_valid_json_file(self, tmp_path, setup_db):
        file = write_log_file(tmp_path, "valid.json", valid_log_data("hello watcher"))
        await process_log_file(str(file))
        rows = await get_recent_logs(limit=10)
        assert len(rows) == 1
        assert rows[0]["message"] == "hello watcher"

    async def test_rejects_invalid_json_content(self, tmp_path, setup_db):
    ...
    async def test_rejects_invalid_schema(self, tmp_path, setup_db):
    ...
    async def test_ignores_non_json_files(self, tmp_path, setup_db):
        txt_file = tmp_path / "readme.txt"
        txt_file.write_text("not a log")
        await process_log_file(str(txt_file))
        rows = await get_recent_logs(limit=10)
        assert len(rows) == 0

    async def test_processes_multiple_files(self, tmp_path, setup_db):
        for i in range(3):
            write_log_file(tmp_path, f"log_{i}.json", valid_log_data(f"event {i}"))
            await process_log_file(str(tmp_path / f"log_{i}.json"))
        rows = await get_recent_logs(limit=10)
        assert len(rows) == 3
...
class TestLogFileHandler:
    ...
    def test_handler_stores_loop(self):
    ...
    def test_on_created_triggers_for_json_file(self):
        ...
        with patch("src.watcher.asyncio.run_coroutine_threadsafe") as mock_bridge:
            mock_bridge.side_effect = lambda coro, loop: coro.close()
...
class TestStartWatcher:
    ...
    def test_watcher_starts_and_stops(self, tmp_path):
        loop = asyncio.new_event_loop()
        config.set_logs_dir(str(tmp_path))
